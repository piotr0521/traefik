...
class PositionCertificateDepositTest extends ApiTestCase
{
    use UsersAwareTrait;
    ...
    protected function setUp(): void
    {
        ...
        $this->positionCertificateDepositRepository = $this->client->getContainer()->get('app.repository.position_certificate_deposit');
        $this->assetCertificateDepositRepository = $this->client->getContainer()->get('app.repository.asset_certificate_deposit');
    ...
    {
        $this->client->request('POST', '/api/position/certificate_deposits', ['json' => []]);
        self::assertJsonContains([
            ...
            'violations' => [
                ...
                3 => [
                    'propertyPath' => 'depositDate',
                    'message' => 'This value should not be blank.',
                ],
                4 => [
                    'propertyPath' => 'depositValue',
                    'message' => 'This value should not be blank.',
                ],
            ],
        ]);
     ...
     * @test
     */
    public function it_shows_error_for_future_deposit_date(): void
    {
        $this->client->request('POST', '/api/position/certificate_deposits', ['json' => [
            'depositDate' => (new DateTime('+1 month'))->format('Y-m-d'),
        ]]);
        self::assertJsonContains([
            'violations' => [
                3 => [
     ...
     * @test
     */
    public function it_creates_new_position_certificate_deposit(): void
    {
        $data = [
            'name' => $this->faker->company,
            'terms' => $this->faker->numberBetween(1, 24),
            'yield' => $this->faker->numberBetween(45, 400),
            'depositDate' => $this->faker->dateTimeBetween('-3 years', '-1 year')->format('Y-m-d'),
            'depositValue' => strval($this->faker->randomFloat(2, 5000, 50000)),
        ...
        $this->client->request('POST', '/api/position/certificate_deposits', ['json' => $data]);
     ...
     * @test
     */
    public function it_only_returns_positions_certificate_deposit_created_by_the_current_user(): void
    {
        $this->trait_it_only_returns_positions_created_by_the_current_user('/api/position/certificate_deposits', $this->positionCertificateDepositRepository);
    }

    /**
     * @test
     */
    public function it_allows_to_get_certificate_deposit_by_id(): void
    {
        $position = $this->positionCertificateDepositRepository->findBy(['createdBy' => $this->getUser('user2')])[0];
        $this->client->request('GET', '/api/position/certificate_deposits/'.$position->getId());
    ...
    {
        $position = $this->positionCertificateDepositRepository->findBy(['createdBy' => $this->getUser('user8')])[0];
        $this->client->request('GET', '/api/position/certificate_deposits/'.$position->getId());
    ...
    {
        $position = $this->positionCertificateDepositRepository->findBy(['createdBy' => $this->getUser('user2')])[0];
        $this->client->request('PATCH', '/api/position/certificate_deposits/'.$position->getId(), [
            'json' => [
                'name' => '',
            ],
     ...
     * @test
     */
    public function it_updates_position_certificate_deposit(): void
    {
        ...
        $position = $this->positionCertificateDepositRepository->find($position->getId());
        self::assertEquals($data['name'], $position->getName());
        self::assertEquals($data['terms'], $position->getTerms());
        self::assertEquals($data['yield'], $position->getYield());
     ...
     * @test
     */
    public function it_deletes_position_certificate_deposit(): void
    {
        $position = $this->getRandomPositionCertificateDeposit($this->getUser('user2'));
        $id = $position->getId();
        $this->client->request('DELETE', '/api/position/certificate_deposits/'.$position->getId());
        $this->assertResponseStatusCodeSame(204);
        self::assertNull($this->positionCertificateDepositRepository->find($id));
    }
    ...
    private function getRandomPositionCertificateDeposit(User $user): PositionCertificateDeposit
    {
        $positions = $this->positionCertificateDepositRepository->findBy(['createdBy' => $user]);
