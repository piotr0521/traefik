...
useBreadcrumbs.forAddPosition(route.params.slug, "Add Public Security");

async function fetchOptions() {
  state.assets = await fetchSymbol(`/api/asset/securities`);
}

async function searchOptions(value) {
  state.assets = await fetchParamsSymbol(`/api/asset/securities`, {
    page: 1,
    symbol: value,
  });
}
...
<template>
  <div class="py-5 px-9 grid">
    <h1 class="text-indigo-900 text-3xl mb-4">Add Public Security</h1>
