...
const [name, privacy, website] = state;
const emit = defineEmits(["added:sponsor"]);
const { getPopupState, setPopupState } = usePopupStore();

function closePopup() {
  setPopupState({ popupName: "showPopupSponsorAdd", value: false });
}

async function onSubmit() {
  if (isSubmitting.value) return;
  const res = await store(new SponsorService());
  if (res) {
    toaster.success("New sponsor added");
    emit("added:sponsor");
    closePopup();
  }
}
...
<template>
  <div
    v-if="getPopupState('showPopupSponsorAdd')"
    ...
    <div class="bg-white p-12 relative z-10 max-w-screen-md w-full rounded-md">
      ...
      <form class="grid gap-5" @submit.prevent="onSubmit">
        ...
        <div class="grid gap-1">
          ...
          <span v-if="meta.privacy.touched && errors.privacy" class="form-error text-sm">
            {{ errors.privacy }}
          </span>
        </div>
        ...
        <div class="grid gap-1">
          ...
          <input
            ...
            v-model="website"
            :class="{ 'border-red-500': meta.website.touched && errors.website }"
            class="form-input"
            type="text"
            name="groshy_sponsor_add[website]"
          />
          <span v-if="meta.website.touched && errors.website" class="form-error text-sm">
            {{ errors.website }}
          </span>
        </div>
