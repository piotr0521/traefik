...
final class PositionLoanFixtures extends BaseFixture implements OrderedFixtureInterface
{
    use MoneyAwareTrait;
    ...
    public function loadData(): void
    {
        ...
        foreach ($users as $userName => $number) {
            ...
            for ($i = 0; $i <= $number; ++$i) {
                $dto = new CreatePositionLoanDto();
                $dto->createdBy = $user;
                $dto->name = $this->faker->realTextBetween(10, 150);
                $dto->interest = $this->faker->randomFloat(2, 2.25, 7.5);
                $dto->loanAmount = $this->fromBase(strval($this->faker->numberBetween(100, 300) * 1000));
                ...
                $dto->loanDate = $this->faker->dateTimeBetween('-3 years', '-2 year');
                $dto->terms = $this->faker->numberBetween(15, 40);

                $position = $this->messageBus->dispatch(new CreatePositionLoanCommand($dto))->last(HandledStamp::class)->getResult();
            }
        }
    }
